...
class SparseTranscoder_OV(HookedRootModule):
    """ """
    ...
    def __init__(
        ...
        self.d_hidden_K = cfg.d_hidden_K
    ...
    def forward(self, x, feature_actsK, pattern):
        ...
        transcoder_out = einops.rearrange(transcoder_out, "... (n_head d_model) -> ... n_head d_model", n_head = self.n_head)
        transcoder_out = einops.einsum(transcoder_out, pattern, "batch posnK n_head d_model, batch n_head posnQ posnK -> batch posnQ n_head d_model")
        transcoder_out = einops.rearrange(transcoder_out, "batch posnQ n_head d_model -> batch posnQ (n_head d_model)")
        
        lp_norm = self.reg_loss(feature_acts)
        gamma_lp_norm = torch.sqrt(self.gamma.float().abs() + self.eps).mean()

        return transcoder_out, feature_acts, lp_norm, gamma_lp_norm
